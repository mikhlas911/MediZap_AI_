...
Deno.serve(async (req: Request) => {
  ...
  const ELEVENLABS_API_SECRET = Deno.env.get('ELEVENLABS_API_SECRET');
  if (!ELEVENLABS_API_SECRET) {
    console.error('[ERROR] ELEVENLABS_API_SECRET environment variable is not set');
    return new Response(
      JSON.stringify({
        success: false,
  ...
  if (!providedSecret || providedSecret !== ELEVENLABS_API_SECRET) {
